...
void Dropdown::addItem(const char* item) {
    if (itemCount_ < MAX_ITEMS) {
        ...
        icons_[itemCount_] = SynthIcon::COUNT;
        itemCount_++;
    }
}
...
int Dropdown::getItemHeight() const {
    switch (style_) {
        case DROPDOWN_COMPACT: return 14;
        default: return h_;
    }
}
...
void Dropdown::drawClassic(Framebuffer& fb, int selectedIndex, bool expanded) {
    ...
    if (selectedIndex >= 0 && selectedIndex < itemCount_ && items_[selectedIndex]) {
        int tx = x_ + 4;
        int ty = y_ + (h_ - 7) / 2;
        if (icons_[selectedIndex] != SynthIcon::COUNT) {
...
void Dropdown::drawUnderlined(Framebuffer& fb, int selectedIndex, bool expanded) {
    if (selectedIndex >= 0 && selectedIndex < itemCount_ && items_[selectedIndex]) {
        ...
        int iconW = (icons_[selectedIndex] != SynthIcon::COUNT) ? 6 : 0;
        int lineY = y_ + h_ - 2;
...
void Dropdown::draw(Framebuffer& fb, int selectedIndex, bool expanded) {
    switch (style_) {
        case DROPDOWN_CLASSIC:    drawClassic(fb, selectedIndex, expanded); break;
        case DROPDOWN_OUTLINED:   drawOutlined(fb, selectedIndex, expanded); break;
        case DROPDOWN_UNDERLINED: drawUnderlined(fb, selectedIndex, expanded); break;
        case DROPDOWN_COMPACT:    drawCompact(fb, selectedIndex, expanded); break;
    }
}
...
bool Dropdown::handleTouch(const TouchState& touch, int& outIndex, bool expanded) {
    ...
    if (touch.x >= x_ && touch.x < x_ + w_ && touch.y >= y_ && touch.y < y_ + h_) {
        outIndex = -1;
        return true;
    }
